...
for i in data['unprefixed']:
	...
	if len(operands_list)>=1:
		...
		if operand_0_reg_id == 99:
			print("")
			print("")
			print("ZZZZZZZZZZZZZZZZ: "+operand0_name)
			print("")
			print("")
...
cbprefixed_count = 0
for i in data['cbprefixed']:
	obj = data['cbprefixed'][i]

	opcode = i

	mne_index = 0
	mne_ID = 99
	for mne_item in cb_mnemonic_list:
		if mne_item==data['cbprefixed'][i]['mnemonic']:
			mne_ID = mne_index
			break
		mne_index = mne_index + 1
	...
	operands_list = data['cbprefixed'][i]['operands']
	operand0_string="NULL"
	operand1_string="NULL"

	if len(operands_list)>=1:
		operand0_obj = data['cbprefixed'][i]['operands'][0]
		op_0_bytes = 0

		operand0_name = data['cbprefixed'][i]['operands'][0]['name']
		operand_0_reg_id = 99
		operand0_name_index = 0
		for item in REGISTER_LIST:
			if operand0_name.upper()==item.upper():
	...
	if len(operands_list)>=2:
		operand1_obj = data['cbprefixed'][i]['operands'][1]
		op_1_bytes = 0
		...
		operand1_name = data['cbprefixed'][i]['operands'][1]['name']
		operand_1_reg_id = 99
		operand1_name_index = 0
		for item in REGISTER_LIST:
			if operand1_name.upper()==item.upper():
	...
	if data['cbprefixed'][i]['immediate']==True:
		immediate_string = "true"
	else:
		immediate_string = "false"

	flag_set = 0
	if data['cbprefixed'][i]['flags']['Z'] == '1':
		flag_set = flag_set | 0b10000000 
	if data['cbprefixed'][i]['flags']['N'] == '1':
		flag_set = flag_set | 0b01000000
	if data['cbprefixed'][i]['flags']['H'] == '1':
		flag_set = flag_set | 0b00100000
	if data['cbprefixed'][i]['flags']['C'] == '1':
		flag_set = flag_set | 0b00010000

	flag_reset = 0
	if data['cbprefixed'][i]['flags']['Z'] == '0':
		flag_reset = flag_reset | 0b10000000 
	if data['cbprefixed'][i]['flags']['N'] == '0':
		flag_reset = flag_reset | 0b01000000
	if data['cbprefixed'][i]['flags']['H'] == '0':
		flag_reset = flag_reset | 0b00100000
	if data['cbprefixed'][i]['flags']['C'] == '0':
		flag_reset = flag_reset | 0b00010000

	print("(*cbprefixed_list)["+str(cbprefixed_count)+"] = *new_inst("+str(opcode)+","+str(mne_ID)+","+str(bytes_num)+","+str(cycles)+","+operand0_string+","+operand1_string+","+immediate_string+","+bin(flag_set)+","+bin(flag_reset)+");")
	cbprefixed_count = cbprefixed_count +1
